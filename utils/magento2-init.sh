...
## This file holds the individual phases of `roll magento2-init`, extracted from the command file
## so each phase can be re-run on its own (e.g. from a project directory left behind by a failed
## run) instead of always restarting the whole scaffold from scratch. `commands/magento2-init.cmd`
## only orchestrates the call order; each function below relies on the same globals the original
## linear script set (PROJECT_NAME, MAGENTO_VERSION, TARGET_DIR, ENV_FILE, the software version
## variables, USED_FALLBACK) so behaviour is unchanged.

## Display usage information for magento2-init
function m2initShowUsage () {
...
## Phase: resolve version -- validate project name and Magento version arguments
function m2initValidateArguments () {
...
## Phase: resolve version -- get compatible software versions based on Magento version (2.4.6+ only)
## Sets: PHP_VERSION, DB_DISTRIBUTION_VERSION, ELASTICSEARCH_VERSION, REDIS_VERSION,
## RABBITMQ_VERSION, VARNISH_VERSION, COMPOSER_VERSION, NODE_VERSION
function m2initResolveSoftwareVersions () {
...
## Phase: create project -- resolve TARGET_DIR from PROJECT_NAME and the optional target directory
## Sets: TARGET_DIR
function m2initResolveTargetDir () {
...
## Phase: create project -- create and enter the project directory
function m2initCreateProjectDirectory () {
...
## Phase: write env -- run env-init and rewrite .env.roll with the resolved software versions
## Requires: PHP_VERSION, DB_DISTRIBUTION_VERSION, ELASTICSEARCH_VERSION, REDIS_VERSION,
## RABBITMQ_VERSION, VARNISH_VERSION, COMPOSER_VERSION (from m2initResolveSoftwareVersions)
## Sets: ENV_FILE
function m2initWriteEnvConfig () {
...
## Phase: install -- sign the SSL certificate for the project
function m2initSignCertificate () {
...
## Phase: install -- start the environment and wait for db/search/redis to become ready
## Requires: ENV_FILE
function m2initStartEnvironment () {
...
## Phase: install -- create the Magento project via composer inside the container, and apply the
## 2.4.4 ReflectionUnionType patch when needed
function m2initInstallMagentoSource () {
...
## Phase: install -- run bin/magento setup:install, with a fallback to Elasticsearch 7 if the
## configured search engine fails
## Requires: ENV_FILE
## Sets: USED_FALLBACK
function m2initSetupMagento () {
...
## Phase: install -- configure base URLs, cache settings and developer mode
## Requires: ENV_FILE, USED_FALLBACK
function m2initConfigureMagento () {
...
## Phase: install -- run initial indexing
function m2initReindexMagento () {
...
## Check if version is 2.4.8 or higher
function m2initIsMagento248OrHigher () {
    local version="$1"
    local base_version
...
## Phase: install -- create the admin user and configure 2FA
function m2initCreateAdminUser () {
    ...
    if m2initIsMagento248OrHigher "${MAGENTO_VERSION}"; then
...
## Print the final summary shown after a successful run
## Requires: USED_FALLBACK
function m2initPrintSummary () {
